...
| `trustedProxyCidrs` | `["127.0.0.0/8", "::1/128"]` | Which peers may supply `clientIpHeader` (default: loopback only). Invalid entries are dropped with an error log and trust narrows to loopback; `0.0.0.0/0` and `::/0` are always rejected                                                                                                                            |
