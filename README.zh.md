...
（16px outline「盾 + 钥匙孔」）；导航行里的图标由宿主绘制，而宿主目前没有给单个设置页配图标的
入口，所以第三方分区仍是默认齿轮。

![改密面板](docs/demo/account-change-password.png)

面板向 `POST /auth/password` 提交（`current` / `password` / `code`，form-urlencoded）；
成功后**该用户的全部会话都会被吊销，包括发起改密的当前会话**。成功文案会停留 2.5 秒，然后把
当前设备送回登录页并带上原因，登录卡片会说明"为什么被登出"：

![改密成功](docs/demo/account-password-changed.png)
