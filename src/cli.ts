...
/** CLI 运行时上下文：参数解析结果 + IO（避免长参数表撑爆函数签名）。 */
interface CliCtx {
  file: string;
  argv: string[];
  io: CliIo;
...
type Patch = (user: UserRecord) => UserRecord;
type Mutator = (snapshot: UsersSnapshot) => void | Promise<void>;
...
export async function main(argv: string[], io: CliIo): Promise<number> {
  ...
  const ctx: CliCtx = { file, argv, io };
  const args = positionals(argv);
  if (args[0] === "skill") return args[1] === "install" ? installSkillCommand(ctx) : usage(io);
  if (args[0] !== "user") return usage(io);
...
function pickFile(argv: string[]): Maybe<string> {
...
function positionals(argv: string[]): string[] {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === "--file") index += 1;
    else if (!token.startsWith("--")) args.push(token);
  }
  return args;
}

/** 明文 `--password` 一律拒绝（P1 §7：口令只走 stdin/交互输入）。 */
function isPasswordFlag(token: string): boolean {
  return token === "--password" || token.startsWith("--password=");
}

/** 锁内变更 + 统一错误出口；mutator 抛错 → 报错并返回 false（不落盘）。 */
async function mutate(ctx: CliCtx, run: Mutator): Promise<boolean> {
  try {
...
async function edit(ctx: CliCtx, name: string, apply: Patch): Promise<boolean> {
...
async function addUser(ctx: CliCtx, name: Maybe<string>): Promise<number> {
  ...
  const added = await mutate(ctx, (snapshot) => {
    if (snapshot.users.has(name)) throw new UsersFileError(`user ${name} already exists`);
...
async function passwdUser(ctx: CliCtx, name: Maybe<string>): Promise<number> {
  if (name === undefined || !USERNAME_RE.test(name)) return usage(ctx.io);
  ...
  if (!(await edit(ctx, name, (current) => ({ ...current, passwordHash: hash })))) return 1;
  ctx.io.out(`user ${name} password changed`);
  if (user.totpSecret !== undefined) ctx.io.out(TOTP_WARNING);
  return 0;
}

/** 锁外预读 users.yaml 快照；读失败 → 报错返回 undefined。 */
async function readSnapshot(ctx: CliCtx): Promise<Maybe<UsersSnapshot>> {
  try {
...
async function findUser(ctx: CliCtx, name: string): Promise<Maybe<UserRecord>> {
  const snapshot = await readSnapshot(ctx);
  const user = snapshot?.users.get(name);
  if (user === undefined && snapshot !== undefined) ctx.io.err(`user ${name} not found`);
  return user;
}

/** 读新口令 + 确认：piped 走一次 readLines(2)，TTY 走注入的 io.readSecret 两次。 */
async function readNewPassword(ctx: CliCtx, fromStdin: boolean): Promise<Maybe<string>> {
...
async function setRole(ctx: CliCtx, name: Maybe<string>, role: Maybe<string>): Promise<number> {
  if (name === undefined || !USERNAME_RE.test(name)) return usage(ctx.io);
  if (role !== "admin" && role !== "user") return usage(ctx.io);
  if (!(await edit(ctx, name, (user) => ({ ...user, role })))) return 1;
  ctx.io.out(`user ${name} role set to ${role}`);
  return 0;
}

async function listUsers(ctx: CliCtx): Promise<number> {
  const snapshot = await readSnapshot(ctx);
