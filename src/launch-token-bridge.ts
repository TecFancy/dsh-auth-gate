 ...
 */
export function makeLaunchTokenBridge(
  ctx: Context,
    ...
    try {
      ...
      if (connection === undefined || typeof connection.authenticatedUrl !== "function") {
        if (!warnedInactive) {
          warnedInactive = true;
          log.warn(
            "launch-token bridge inactive: no connection.authenticatedUrl (dsh < 0.1.2-alpha?)",
          );
        }
        return Promise.resolve(undefined);
      }
      // 固定 loopback base 仅用于触发 authenticatedUrl 的 URL 解析；返回值中的
      // host/scheme/其它 query 全部丢弃，只保留 launch token。
      const token = new URL(connection.authenticatedUrl("http://127.0.0.1")).searchParams.get(
        "token",
      );
      if (token === null || token === "") {
        if (!warnedError) {
          warnedError = true;
          log.warn("launch-token bridge unavailable: authenticatedUrl returned no token");
        }
        return Promise.resolve(undefined);
      }
      return Promise.resolve(`/?token=${encodeURIComponent(token)}`);
    } catch (error) {
      if (!warnedError) {
        warnedError = true;
        log.warn(
          `launch-token bridge unavailable: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      return Promise.resolve(undefined);
    }
