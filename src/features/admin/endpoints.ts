...
import type { AdminDeps, AdminRoutes, ResetInput } from "./deps.js";
...
export function makeAdminRoutes(deps: AdminDeps): AdminRoutes {
  return {
...
async function listUsers(deps: AdminDeps, res: ServerResponse, session: Session): Promise<void> {
...
export async function handleResetPassword(
  ...
  const input: ResetInput = { actor, password, code, secrets };
  if ((await resetTarget(deps, res, ctx, input)) === undefined) return;

  const hashed = await hashOr503(deps, res, ctx, input);
  if (hashed === undefined) return;
  if (!(await writeReset(deps, res, ctx, hashed, input.secrets))) return;
...
): Promise<ResetInput["actor"] | undefined> {
