 ...
 */
export function buildChallengeValue(
  username: string,
...
export function parseChallengeValue(
  ...
  if (lastDot <= 0 || lastDot === value.length - 1) return undefined;
  ...
  if (providedMac.length !== computedMac.length) return undefined;
  if (!timingSafeEqual(Buffer.from(providedMac), Buffer.from(computedMac))) return undefined;

  const dot = prefix.lastIndexOf(".");
  if (dot <= 0 || dot === prefix.length - 1) return undefined;
