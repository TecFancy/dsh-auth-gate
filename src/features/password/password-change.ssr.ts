 ...
 */
export function passwordChangePageHtml(notice: string | undefined): string {
  const noticeHtml =
    notice === undefined || notice === "" ? "" : `<p class="notice" role="status">${notice}</p>`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
function hasSession(deps: PasswordChangePageDeps, req: IncomingMessage): boolean {
  const store = deps.sessions();
  const token = parseCookieHeader(req.headers.cookie, deps.cookieName);
  if (store === undefined || token === undefined || token === "") return false;
 ...
 */
export function handlePasswordChangePage(
  deps: PasswordChangePageDeps,
  req: IncomingMessage,
  res: ServerResponse,
): void {
 ...
 */
export function respondChanged(
  deps: ChangeResponseDeps,
  res: ServerResponse,
  navigation: boolean,
): void {
  res.setHeader("cache-control", "no-store");
