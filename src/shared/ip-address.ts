...
function stripDecorations(raw: string): string {
  ...
  if (bracketed !== null) return (bracketed[1] ?? "").split("%")[0] ?? "";
  if (isIP(text) === 0) {
    const withPort = /^(\d{1,3}(?:\.\d{1,3}){3}):\d+$/.exec(text);
    if (withPort !== null) return withPort[1] ?? "";
  }
  return text.split("%")[0] ?? "";
}

/** 严格点分四段（拒绝前导零与 >255）。 */
function ipv4ToBytes(address: string): number[] | undefined {
  const parts = address.split(".");
  if (parts.length !== 4) return undefined;
  const bytes: number[] = [];
  for (const part of parts) {
    if (!/^(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)$/.test(part)) return undefined;
...
/** 单段十六进制组（1-4 位）。 */
function groupsOf(part: string): number[] | undefined {
  if (part === "") return [];
  const groups: number[] = [];
...
function expandEmbeddedIpv4(body: string): string | undefined {
  const embedded = /((?:\d{1,3}\.){3}\d{1,3})$/.exec(body);
  if (embedded === null) return body;
...
function bytesToAddress(bytes: number[]): string {
...
function addressToBytes(address: string): number[] | undefined {
  if (address === "") return undefined;
  if (isIP(address) === 4) return ipv4ToBytes(address);
...
export function parseCidr(raw: string | undefined): CidrNetwork | undefined {
  const text = (raw ?? "").trim();
...
export function cidrContains(network: CidrNetwork, address: string): boolean {
