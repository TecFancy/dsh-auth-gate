...
description: Use when the user asks about dsh-auth-gate configuration - supported options (mode/totp/sessionTtl/cookieName/tokenRef/cookieSecure/usersFile/publicHost/revokeSweepMs/clientIpHeader/trustedProxyCidrs/logoutOrder), how to configure them, the dsh-auth CLI (incl. user totp enable/disable), login trouble, rate limiting, or the logout button. Ships with the dsh-auth-gate package; install with `dsh-auth skill install`. / 用户询问 dsh-auth-gate 配置（mode/totp/sessionTtl/cookieName/tokenRef/cookieSecure/usersFile/publicHost/revokeSweepMs/clientIpHeader/trustedProxyCidrs/logoutOrder）、dsh-auth CLI（含 user totp enable/disable）、登录故障、限速或退出按钮时使用。随 dsh-auth-gate 包发布；用 `dsh-auth skill install` 安装。
...
- Disable blocks new logins and, in password mode, strips live sessions within
  `revokeSweepMs`; `0` turns the sweep off (sessions then only expire). /
  禁用挡新登录，并在 `revokeSweepMs` 窗口内收回已签发会话；`0` 关闭扫描（会话只等到期）。
...
| Repeated failures locked with 429                             | Rate limit: 5 failures locks for 30s (resets on restart); lockout shows the same card with the retry seconds, scoped to "this network"; remaining attempts are never shown. / 限速：5 次失败锁 30s（重启清零）；锁定页同卡显示剩余秒数与「此网络」文案，从不显示剩余次数                                                                                                                                                                              |
| Everyone gets 429 after one device mistyped a password        | Same-host reverse proxy (Caddy / Tunnel / nginx): every request arrives from the proxy address, so all clients share one lockout bucket. Set `clientIpHeader: "x-forwarded-for"` (`cf-connecting-ip` behind Cloudflare) and make the proxy **overwrite** that header (D19). / 同主机反代下所有请求都来自反代地址、共用一个锁定桶 → 配 `clientIpHeader`（Cloudflare 在边缘时用 `cf-connecting-ip`），并让反代**覆盖写入**该头（D19）                   |
| Login page renders but login returns 401                      | Wrong username/password re-renders the login card with an inline `Invalid username or password.` (username kept, password retyped); known-good password still 401s -> check `users.yaml` path/permissions; with JS refresh no longer spends a failure. / 用户名/密码错误重渲登录卡片并内联显示该常量（用户名保留、密码重敲）；密码正确仍 401 -> 查 users.yaml 路径/权限；有 JS 刷新不再耗一次失败                                                     |
...
| A disabled user keeps browsing                                | Password mode revokes issued sessions on the `revokeSweepMs` cadence (default 5000ms); with `0` the disabled user stays signed in until the cookie expires. / 禁用后仍能访问：password 模式按 `revokeSweepMs`（默认 5000ms）周期收回会话；配 `0` 时要等 cookie 过期才掉线                                                                                                                                                                             |
