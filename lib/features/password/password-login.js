import { validateNext, parseFormBody, parseCookieHeader, resolvePublicHost, totpChallengePageHtml, } from "../../shared/index.js";
import { DUMMY_HASH } from "./password.js";
import { LoginRateLimiter, loginPath } from "../../shared/index.js";
import { buildSetCookie } from "../../session/index.js";
import { issueLoginSession } from "./session-issue.js";
import { INVALID_TOTP_CODE, sendInvalidCredentials, sendLockout, sendTotpLockout, } from "./login-failure-pages.js";
...
async function handleTotpSubmit(deps, host, res, username, code, next, ip) {
...
async function handlePasswordSubmit(deps, res, params, next, ip, host) {
    ...
    if (loaded === undefined)
        return; // 系统错误不计失败
    if (await rejectedInvalid(deps, res, loaded, username, password, ip, accountKey, { host, next })) {
        return;
    }
    const user = loaded.snapshot.users.get(username);
...
function rejectTotp(deps, host, res, next, username) {
    deps.logger.info("login rejected");
    res.setHeader("cache-control", "no-store");
    res.writeHead(401, { "content-type": "text/html; charset=utf-8" });
    res.end(totpChallengePageHtml(next, INVALID_TOTP_CODE, {
        host,
        who: username,
...
function requireStore(deps, res) {
...
async function rejectedInvalid(deps, res, loaded, username, password, ip, accountKey, page) {
...
function lockoutSeconds(deps, ip, accountKey) {
