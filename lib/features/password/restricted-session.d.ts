 ...
 */
export declare const RESTRICTED_SESSION_KIND = "password-change-only";
/**
 * 受限会话 TTL：15 分钟（CONTRACT §3），**不续期**（会话层本就没有滑动过期）。
 * 到期 = 按普通未认证处理：重新登录时标记仍在 → 再发一枚受限会话，闭环正确。
 */
export declare const RESTRICTED_SESSION_TTL_SECONDS: number;
/**
 * 受限会话被拒的导航目标（§3）：改密页，**不是** `/auth/login`
 * （回登录页会渲染第二个登录表单，受限闭环会断）。
 */
export declare const RESTRICTED_REDIRECT_PATH = "/auth/password";
/**
 ...
 */
export declare function isRestrictedSession(session: Session | undefined): boolean;
//# sourceMappingURL=restricted-session.d.ts.map