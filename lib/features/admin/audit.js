...
function stripControlChars(value) {
    ...
    for (const char of value) {
        if (isInvisible(char.codePointAt(0) ?? 0))
            continue;
        out += char;
    }
    return out;
...
function clipTarget(target) {
    ...
    if (stripped.length <= TARGET_LIMIT)
        return stripped;
 ...
 */
export function auditDenied(logger, ctx, reason) {
    const event = {
    ...
    if (ctx.actor === "" || reason === "unauthenticated")
        logger.info(event);
    else
        logger.error(event);
}
 ...
 */
export function auditSuccess(logger, ctx, sessionsRevoked) {
    const event = {
        event: RESET_EVENT,
        ...ctx,
        target: clipTarget(ctx.target),
