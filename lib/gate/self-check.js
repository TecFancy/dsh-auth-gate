import { isGuarded } from "./guard.js";
/**
...
export function assertGuarded(server) {
    ...
    for (const [path, route] of server.exact) {
        if (!isGuarded(route.handler))
            failures.push(`exact ${path}`);
    }
    for (const [path, route] of server.prefixes) {
        if (!isGuarded(route.handler))
            failures.push(`prefix ${path}`);
    }
    for (const [path, route] of server.upgrades) {
        if (!isGuarded(route.handler))
            failures.push(`upgrade ${path}`);
    }
    ...
    for (const method of ["register", "registerUpgrade", "registerFallback"]) {
        ...
        const fn = Reflect.get(server, method);
        if (!isGuarded(fn))
            failures.push(`method ${method}`);
    }
    return failures;
}
