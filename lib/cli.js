...
import { checkPasswordPolicy, compareNames, defaultUsersFilePath, loadUsersFile, mutateUsersFile, USERNAME_RE, UsersFileError, } from "./shared/index.js";
 ...
 */
async function setDisabledUser(ctx, name, disabled) {
    if (name === undefined)
        return usage(ctx.io);
    if (!(await edit(ctx, name, (user) => ({ ...user, disabled }))))
        return 1;
