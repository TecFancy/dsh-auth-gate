...
| [`implemented/`](implemented/)                              | 已交付规格与规划：impl-m1~m4（当前实现基准，m4 最新）、impl-m2-admin（P2/D25 管理面切片）、impl-launch-token-bridge（0.1.2-alpha 兼容层）、impl-client-logout、login-page-polish-plan（已落地视觉基线）、totp-fix-plan（0.11.1 修复计划，已实施） | 核对「当前行为依据」时    |
| [`plans/`](plans/)                                          | 进行中规划：pr53-login-page-fix-plan（#53 移植）                                                                                                                                                                                                  | #53 落地时                |
| [`decisions.md`](decisions.md) + [`decisions/`](decisions/) | 重大决策编号索引 D1–D25（双语记录，proposed / implemented / archived）                                                                                                                                                                            | 理解「为什么」时          |
| [`demo/`](demo/)                                            | README 效果图（登录页 / TOTP 验证码页 / 实例 / 设置导航「账号安全」行 / 改密面板 / 用户管理块 / 重置成功态；**统一英文界面 + 浅色主题**，中英共用同一套）                                                                                         | README 引用               |
| [`reviews/`](reviews/)                                      | 外部评审记录（grok-4.6 launch-token bridge review 等）                                                                                                                                                                                            | 变更合入前对照            |

## 阅读路径

- **部署/运维**：`deployed/deployment.md`（首次部署走 §0–§4 验收；升级走 §5 + §5.1）→ `deployed/reverse-proxy.md` → `deployed/local-proxy.md`；升级后复跑入口覆盖脚本的判据见 `deployed/entry-coverage-0.1.5-rc.2.md` §5
