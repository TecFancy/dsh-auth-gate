...
验证只在当前宿主版本（0.1.5-rc.2）做过。README 配图：导航列特写
`docs/demo/account-nav-icon.en.png`（能同时看到宿主齿轮与我们盾牌）；全部 README 截图统一使用英文
界面，中英两版共用一套（D24.3）。改密成功后的兜底面板只在浏览器拒绝跳转时出现，实测在正常浏览器
里无法复现（网络层拦截会直接落地成错误页），因此不留截图，该分支由
`src/client/account-relogin.test.tsx` 钉住。

## 补充修订（D24.3，2026-09-24）

README 上线后主人提了三件事：截图不能中英混杂、中文版不能读起来像逐句直译、npm 上的介绍和
GitHub 仓库描述要和 README 保持一致。

1. **截图统一成英文一套。** D24.2 交付的 `*.en.png` / `*.zh.png` 双语图对取消：同一页里英文的
   服务端渲染卡片和中文面板混在一起像事故现场，而评估者第一眼看到的本来就是英文界面。两版 README
   现在引用同一套英文界面、浅色主题的资产，`*.zh.png` 全部删除；新的不变量（两版图片集合相同、
   `docs/demo/` 不再允许出现 `*.zh.png`）由 parity 门禁强制。
2. **原生中文，不是翻译腔。** `README.zh.md` 按中文技术写作习惯逐句重写：骨架、事实、链接都不变，
   去掉「版本走廊」（version corridor）、「修复回归」、「登录卡」这类直译，并统一术语（全篇用
   「密码」，不再密码与口令混用）。由 grok-4.6 以双语编辑身份评审。
3. **重写时抓到的结构漂移。** 中文配置表少了英文表自 D19 起就有的两行（`clientIpHeader`、
   `trustedProxyCidrs`）；原门禁只数列表项、围栏代码块和 `###`，看不见表格行。门禁现在同时比较
   表格行数与每行列数。

**D24.3 残留**：兜底成功面板仍然没有截图（只有浏览器层拒绝跳转才到得了，网络层拦截会直接落地成
错误页，见 D24.1 残留）；npm 的 `description` 要等下次发版（0.15.0）才会进 registry，而 GitHub
仓库描述与 topics 属于仓库元数据，可以随时单独更新。
