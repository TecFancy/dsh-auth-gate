...
#    login → enter the instance; a Sign out icon button sits at the top-right — in the session
#    header (right of Session log) inside a session, and at the window's top-right corner on the
#    new-session page (client half, 0.6.5+), or visit /auth/logout?next=/ via URL to log out.
```
