...
> - **§3.3 / 文件地图**：`CHALLENGE_COOKIE` / 构建 / 解析 / 签发均在
>   `src/features/password/challenge-cookie.ts`（0.11.1 拆分；经 `password-login.ts` 再导出）。
>   正文中把 `password-endpoints.ts` 当作 cookie 归属的表述（§3.3、T11、文件地图）指
>   0.11.1 之前的布局。
...
> 基线：`docs/implemented/impl-m3_zh.md`（M3 已交付：users.yaml + scrypt + 限速 + `dsh-auth user` CLI——M4 在其上叠加）。
> M1 的 D1–D16、M2 的 M1–M22 和 M3 的 P1–P26 除非下文明确修订，否则保持不变。
> 设计依据：`docs/specs/dsh-auth-plan_zh.md` §6 阶段 3 / §8；工程门禁与 slice（切片）布局：`docs/specs/development.md`
> 和 `docs/specs/src-refactor-plan.md`（分层 `src/`：gate/session 核心 + features/{token,password,proxy} + shared 叶子；
> 跨 slice 导入只能走 barrel（桶/聚束导出）；features 之间互不导入）。
> **本文件是 M4 细则的唯一权威**；与 plan/M1/M2/M3 冲突时，以本文件为准。
>
> 环境与验证工作流：见 `docs/handoff/handoff-m3_zh.md`（新 session 必读：服务器冒烟工作流 §4、M1–M3 踩坑 §3、
...
| T16 | DoD               | (1) `npm run verify` 全绿（全部 10 个门禁；230 + 新测试）。(2) `lib/` 在同一提交中重建。(3) **otplib 交叉验证**（§3.1）：≥ 200 个随机用例 + RFC 向量，逐位一致，transcript 记入 handoff-m4。(4) 在 `web-test` 上做真实服务器冒烟（handoff-m3 §4 工作流）：密码 → 挑战页 → 错误验证码 401 → 正确验证码 → 会话 cookie；挑战 cookie 被清除；重放（同一验证码两次）→ 401；required 模式下无 secret 的用户 → 401；`mode:"token"` 回归不受影响。(5) README/README.zh 更新：配置表 + CLI + 局限（内存防重放/限速器重启清零；挑战 cookie TTL；`off` 默认）。(6) ADR 落地 + decisions.md 更新。(7) 写 `docs/handoff/handoff-m4.md`（环境事实、真实冒烟结果、交叉验证 transcript、踩坑、M5 起步提示；zh 镜像可选）。 |
...
- **与 otplib 交叉验证（用户已批准 2026-08-30）**：里程碑落地前，运行一个一次性临时脚本（放临时目录、
  ...
  运行时依赖：脚本的 transcript 记入 `docs/handoff/handoff-m4.md`；该包不加入 package.json。
...
docs/implemented/impl-m4.md  （本文件）+ docs/implemented/impl-m4_zh.md（镜像，同一批）
...
docs/handoff/handoff-m4.md                 给 M5 的 handoff（en；zh 镜像可选）
```
...
10. ADR（T15）+ decisions.md + README zh/en + docs/implemented/impl-m4_zh.md；最终 verify。
