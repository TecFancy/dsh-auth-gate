...
1. 注册形状：token 模式 4 条路由（prefix `/auth`、exact `/auth/login`、`/auth/logout`、`/auth/status`，M15）；**D22 修订（2026-09-23）**：password 模式的 `registerPasswordEndpoints` 注册第五条路由 exact `/auth/password`（仅当传入可选 `passwordChange` deps；未传时必须**不**注册，exact 路由表断言要能拦住这种退化）。
