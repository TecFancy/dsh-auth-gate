...
| M5  | 端点集       | `GET /auth/login`（自包含 HTML）、`POST /auth/login`（urlencoded：`token` + `next`）、`POST /auth/logout`、`GET /auth/status`（JSON `{ authenticated: boolean }`）。路由模型见 M15（3 exact + 1 prefix 兜底；**D22 修订（2026-09-23）**：password 模式下路由集为 **4 exact + 1 prefix 兜底**，第四条 exact 是 `/auth/password`，token 模式仍注册 3 exact）；`/auth/status` **只认 cookie**（Bearer 不参与：无状态通道不建会话，不是"会话状态"）**D25（2026-09-24）**：见 [impl-m2-admin_zh.md](./impl-m2-admin_zh.md) §4.5b                                                           |
...
| M15 | 路由模型     | webserver **无 method 路由**（`exact` 表只按 pathname 建键、重复 path 抛错，实测）→ **password 模式下 4 条 exact 路由**（**D22 修订（2026-09-23）**：`/auth/login`、`/auth/logout`、`/auth/status`、**`/auth/password`**（由 D22 新增，仅在传入可选 `passwordChange` deps 时注册）；token 模式仍为 **3 条 exact**）+ **1 条 prefix 兜底**（`/auth` → 404，防未注册 `/auth/*` 落到 SPA fallback，M20）；每个 exact handler 内部按 `req.method` 分发，非白名单 method → `405` + `allow` 头 + `text/plain`；**D25（2026-09-24）**：见 [impl-m2-admin_zh.md](./impl-m2-admin_zh.md) §4.5b |
...
**D22 修订（2026-09-23）**：password 模式下 exact 集新增第四条路由 `/auth/password`
（`registerPasswordEndpoints` 仅在传入可选 `passwordChange` deps 时注册它；`index.ts` 在 password
模式下**总是**注入），故该模式为 1 prefix + 4 exact。token 模式不变（1 prefix + 3 exact），
下文关于 M2 三条端点的全部陈述在两种模式下依然成立。

**P2 管理面（D25，2026-09-24）**：password 模式到 1 prefix + **6 exact**；D25 的路由/端点修订
与实施期复审收紧已移至 [impl-m2-admin_zh.md](./impl-m2-admin_zh.md) §4.5b。

- **prefix `AUTH_PATH_PREFIX`（= `"/auth"`，兜底，M20）**：恒 `404` + `content-type: text/plain` +
  no-store：未注册的 `/auth/*` 不得落到 SPA fallback（exact 优先于 prefix，不遮蔽下面的 exact 端点；password 模式下包含 D22 的第四条路由 `/auth/password`）。
...
store.getByToken(token) !== undefined;`
  ...
  **D25（2026-09-24）**：已认证时的加法字段已移至
  [impl-m2-admin_zh.md](./impl-m2-admin_zh.md) §4.5b.1。
...
1. 注册形状：token 模式 4 条路由（prefix `/auth`、exact `/auth/login`、`/auth/logout`、`/auth/status`，M15）；**D22 修订（2026-09-23）**：password 模式的 `registerPasswordEndpoints` 注册第五条路由 exact `/auth/password`（仅当传入可选 `passwordChange` deps；未传时必须**不**注册，exact 路由表断言要能拦住这种退化）。**D25（2026-09-24）**：password 模式注册 7 条（1 prefix + 6 exact）；P2 增量已移至 [impl-m2-admin_zh.md](./impl-m2-admin_zh.md) §5b。
