...
| P9  | 认证结果       | 未知用户 → 对 DUMMY_HASH 验证后 401；口令错误 → 401；`disabled: true` → **仍跑一次真验证**（时序均匀）后 401。三者响应体统一 `"invalid credentials"` + `logger.info("login rejected")`（**不含用户名**——防枚举，日志纪律 P23）；**D20 修订（2026-09-22）**：状态码 401、「三态共用唯一常量」、「绝不反射请求文本」、「日志不含用户名」全部不变，但 body 不再是 `text/plain`：改回渲染登录卡片（`content-type: text/html; charset=utf-8`，仍 `no-store`），error slot 填唯一常量 `Invalid username or password.`（旧 body 字符串 `invalid credentials` 已消失）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| P10 | 登录限速       | 内存态 `LoginRateLimiter`（**重启清零**，README 注明）。常量：`maxFailures=5`、`baseDelaySeconds=30`、`maxDelaySeconds=900`、`windowSeconds=600`；第 n（n≥5）次失败 → 锁定 `min(30 * 2^(n-5), 900)` 秒。锁定期请求 → **429** + `retry-after: <秒>` + text/plain `"too many attempts"` + `logger.info("rate limit exceeded")`，**不增计数、不验证**；锁到期条目清零重计；**失败计数 10 分钟无失败后衰减清零**（滑动窗口，`windowSeconds`）；成功清双桶。IP 默认取归一化后的 `req.socket.remoteAddress ?? ""`，**默认不读任何转发头**；**D19 修订（2026-09-22）**：配置了 `clientIpHeader` 且 peer ∈ `trustedProxyCidrs`（默认回环；无对端地址的 Unix socket 视同本机）时才读该头，取从右往左数第一个「合法且非受信」的地址；**任何一段不是合法 IP 则整头作废、回退 socket 地址**，其余情况仍忽略可伪造输入；`username === ""` 只计 IP 桶；条目上限 10000，超限删最早插入；**D20 修订（2026-09-22）**：429 的 body 也改为 HTML——同一张登录卡片，文案为 `Too many sign-in attempts from this network. Try again in N seconds. There is no password reset, so ask the instance owner if you are stuck.`（N = 当前 `retry-after` 整数，N=1 时用单数 `second`），TOTP 第二段渲染挑战卡 + 同一文案；状态 429 + `retry-after` 不变，提交按钮**有意不在 HTML 里 disabled**（无 JS 的浏览器仍可重试），永不渲染倒计时或「剩余次数」 |
| P11 | mode 语义      | `mode: "password"` 激活 PasswordGate + password 端点（**不再抛错**）；`mode: "token"` 为默认且行为与 M2 **逐字节一致**（`token-gate.ts` 零改动、credentials 接线零改动；**D21 修订（2026-09-23）**：「逐字节一致」指的是门与 credentials 接线，不含 token 登录失败的浏览器表示 —— token 401 的 body 现为登录卡片，见 D21，`token-gate.ts` 与 credentials 接线仍零改动）。两模式**二选一不可并存**；password 模式不读 credentials 服务、`tokenRef` 配置被忽略                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| P14 | 登录端点       | `POST /auth/login`（password 模式）字段 `username`/`password`/`next`，复用 `parseFormBody`（415/413 语义与 M2 完全一致）。成功 → `store.create(username, ttl*1000)` + `buildSetCookie` 4 参 + 302 next + `info("session issued")`。失败 → 401 `"invalid credentials"`；`loadUsers` 失败 → 503 `"user store unavailable"` + `error`；`sessions()` undefined → 503 `"session store unavailable"` + `error`（M2 文案）。429 见 P10。全程 no-store；**D20 修订（2026-09-22）**：失败改为渲染 401 登录卡片 HTML（`Invalid username or password.`，提交的用户名以转义后的 `value=` 回填、口令永不回填），不再是 `text/plain "invalid credentials"`；429 渲染同一张卡片 + 锁定文案（P10）；用户名在查用户、跑 DUMMY_HASH 验证（P3）之前 trim，口令不 trim；401/429 状态码与 no-store 不变；token 模式的 `text/plain` 401 当时未动（M2 冻结）—— **已被 D21（2026-09-23）取代**：token 模式 401 现在渲染自己的登录卡片（唯一常量 `Invalid access token.`），见 D21                                                                                                                                                                                                                                                                                                                                                                 |
...
| P16 | 路由模型       | password 模式注册**同样的 4 条**（prefix `/auth` 404 兜底 + 3 exact），由 `index.ts` 按 mode 二选一装配（不同时注册两条相同 path：webserver 会抛重复）。logout/status 行为与 M2 **完全一致**：logout 仅 POST、next 仅 query、无 body 可幂等调用、`Max-Age=0`；status 仅 GET、**只认 cookie**（Bearer 会话 token 不参与：无状态通道不建会话）。**D22 修订（2026-09-23）**：password 模式现注册第五条路由，即 prefix `/auth` + **4 条 exact**，第四条 exact 为 `/auth/password`（仅在传入可选 `passwordChange` deps 时注册；`index.ts` 在 password 模式下总是注入）；「不同时注册两条相同 path」与 P16 其余语义不变。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
2. `username = params.get("username") ?? ""`；`password = params.get("password") ?? ""`；
   ...
   `ip = deps.clientIp?.(req) ?? req.socket.remoteAddress ?? ""`（**D19**：注入的解析器负责归一化，
   并在配置了受信反代头时改为读该头）。
3. `const check = deps.limiter.check(ip, username === "" ? undefined : username)`；
   ...
   no-store + `info("rate limit exceeded")`；**return（不验证、不增计数）**。**D20 修订（2026-09-22）**：429 body 改为登录卡片 HTML（`sendLockout`；TOTP 第二段用 `sendTotpLockout` 渲染挑战卡），文案换成 `Too many sign-in attempts from this network. Try again in N seconds. There is no password reset, so ask the instance owner if you are stuck.`，不再是 `text/plain "too many attempts"`；状态码与 `retry-after` 不变。
...
6. `if (!ok || user === undefined || user.disabled) { deps.limiter.recordFailure(ip, username === "" ? undefined : username);`
   → 401 `"invalid credentials"` + no-store + `info("login rejected")`；return。`}`。**D20 修订（2026-09-22）**：401 body 改为登录卡片 HTML——`sendInvalidCredentials` 用常量 `Invalid username or password.` 填 error slot，回填转义后的用户名（口令不回填）、`<title>` 加 `Error: ` 前缀；401、`no-store` 与该日志行不变。步骤 2 读到的 `username` 在步骤 5 查用户与 DUMMY_HASH 验证前 trim（口令不 trim）。
...
结构照 `auth-endpoints.ts`（合并 disposer、track 收集）：M3 基线 4 条路由。（**D22 修订（2026-09-23）**：password 模式下为 5 条，即 prefix `/auth` + **4 条 exact**，第四条是 `/auth/password`；仅在传入可选 `passwordChange` deps 时注册）：
...
- **D23 修订（2026-09-23）**：CLI 的变更命令（`add` / `disable` / `role` / `passwd`）统一改走
  `mutateUsersFile`（锁内 RMW + 写前 CAS + `.bak`，见 D23），因此上面三处 `writeUsersFile` 按
  「经 `mutateUsersFile` 的原子写」理解；`readLine` 由「每次新建 readline 读一行」换成
  `readLines(n)`（一次会话读 n 行，`user passwd` 分两次读入口令的前提，旧写法在管道下第二行恒为空）；
  `disable` 的未知用户文案统一为 `user <name> not found`（与 `passwd` / `role` / `totp` 一致）。
...
4. POST login 失败：verify false → 401 `"invalid credentials"` + `info("login rejected")` +
   ...
   - 401；禁用用户 → verify 收到该用户真哈希 + 401（P9：三态统一）。（**D20 修订（2026-09-22）**：401 body 现为 HTML 卡片——断言 `content-type: text/html; charset=utf-8`、常量 `Invalid username or password.`、回填的用户名与 `Error: ` 标题前缀，而非 `text/plain "invalid credentials"`。）
...
6. POST login 503：fake loadUsers reject → 503 `"user store unavailable"` + error 日志 + 不计
   ...
   第二次登录不再重复 warn（模块级 flag）。（**D20**：与第 4 条相同的 HTML 卡片与常量 `Invalid username or password.`。）
...
3. 禁用/未知用户：disableduser → 401；`ghost` → 401（响应体一致 `invalid credentials`）。（**D20 修订（2026-09-22）**：响应体改为一致的 HTML 卡片，常量 `Invalid username or password.`。）
