...
| P14 | 登录端点       | `POST /auth/login`（password 模式）字段 `username`/`password`/`next`，复用 `parseFormBody`（415/413 语义与 M2 完全一致）。成功 → `store.create(username, ttl*1000)` + `buildSetCookie` 4 参 + 302 next + `info("session issued")`。失败 → 401 `"invalid credentials"`；`loadUsers` 失败 → 503 `"user store unavailable"` + `error`；`sessions()` undefined → 503 `"session store unavailable"` + `error`（M2 文案）。429 见 P10。全程 no-store；**D20 修订（2026-09-22）**：失败改为渲染 401 登录卡片 HTML（`Invalid username or password.`，提交的用户名以转义后的 `value=` 回填、口令永不回填），不再是 `text/plain "invalid credentials"`；429 渲染同一张卡片 + 锁定文案（P10）；用户名在查用户、跑 DUMMY_HASH 验证（P3）之前 trim，口令不 trim；401/429 状态码与 no-store 不变；token 模式的 `text/plain` 401 未动（M2 冻结）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
