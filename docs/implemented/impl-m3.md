...
| P9  | Auth result            | Unknown user → verify against DUMMY_HASH then 401; wrong password → 401; `disabled: true` → **still run one real verification** (timing uniformity) then 401. All three share the response body `"invalid credentials"` + `logger.info("login rejected")` (**no username** — anti-enumeration, log discipline P23); **D20 amendment (2026-09-22):** status 401, "one constant shared by all three states", "no reflection of request text" and "no username in logs" are all unchanged, but the body is no longer `text/plain`: the login card is re-rendered (`content-type: text/html; charset=utf-8`, still `no-store`) with the error slot filled by the single constant `Invalid username or password.` (the old body string `invalid credentials` is gone)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| P10 | Login rate limiting    | In-memory `LoginRateLimiter` (**reset on restart**, note in README). Constants: `maxFailures=5`, `baseDelaySeconds=30`, `maxDelaySeconds=900`, `windowSeconds=600`; the nth (n≥5) failure → lock for `min(30 * 2^(n-5), 900)` seconds. Request during the lock period → **429** + `retry-after: <seconds>` + text/plain `"too many attempts"` + `logger.info("rate limit exceeded")`, **does not increment counters, does not verify**; on lock expiry the entry is cleared and counted from zero; **failure count decays to zero if no failure occurs for 10 minutes** (sliding window, `windowSeconds`); success clears both buckets. IP is taken from `req.socket.remoteAddress ?? ""` (normalized), **no forwarding header is read by default**; **D19 amendment (2026-09-22):** a configured `clientIpHeader` is read only when the peer is inside `trustedProxyCidrs` (loopback by default; a peer without an address, i.e. a Unix socket, counts as local), taking the first address from the right that is both a valid IP and not a trusted hop; **any segment that is not a valid IP invalidates the whole header and falls back to the socket address**, so forgeable input stays ignored otherwise; `username === ""` only counts the IP bucket; entry cap 10000, when exceeded delete the earliest inserted; **D20 amendment (2026-09-22):** the 429 body is now HTML too — the same login card carrying `Too many sign-in attempts from this network. Try again in N seconds. There is no password reset, so ask the instance owner if you are stuck.` (N = the `retry-after` integer, singular `second` when N = 1), and the TOTP second stage renders the challenge card with the same message; status 429 + `retry-after` are unchanged, the submit button is deliberately **not** disabled in HTML (a browser without JavaScript can still retry), and neither a live countdown nor an "attempts remaining" count is ever rendered |
| P11 | mode semantics         | `mode: "password"` activates PasswordGate + password endpoints (**no longer throws**); `mode: "token"` is the default and behaves **byte-for-byte** the same as M2 (`token-gate.ts` zero changes, credentials wiring zero changes; **D21 amendment (2026-09-23):** this byte-for-byte clause covers the gate and the credentials wiring, not the browser representation of a failed token login - the token 401 body is now the login card, see D21, and `token-gate.ts` plus the credentials wiring remain untouched). The two modes are **mutually exclusive, never simultaneous**; password mode does not read the credentials service, and the `tokenRef` configuration is ignored                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| P14 | Login endpoint         | `POST /auth/login` (password mode) accepts fields `username`/`password`/`next`, reuses `parseFormBody` (415/413 semantics exactly like M2). Success → `store.create(username, ttl*1000)` + `buildSetCookie` 4 args + 302 next + `info("session issued")`. Failure → 401 `"invalid credentials"`; `loadUsers` failure → 503 `"user store unavailable"` + `error`; `sessions()` undefined → 503 `"session store unavailable"` + `error` (M2 wording). 429 see P10. no-store throughout; **D20 amendment (2026-09-22):** failure now renders the 401 login-card HTML (`Invalid username or password.`, submitted username echoed back HTML-escaped, password never echoed) instead of `text/plain "invalid credentials"`, and the 429 renders the same card with the lockout message (P10); the username is trimmed before the user lookup and before the DUMMY_HASH verification (P3), the password is never trimmed; 401/429 status codes and the no-store rule are unchanged, and token mode's 401 body was still `text/plain` at this point (frozen by M2) — **superseded by D21 (2026-09-23):** token mode now renders its own login card on a 401 (single constant `Invalid access token.`), see D21                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| P16 | Route model            | password mode registers **the same 4** (prefix `/auth` 404 fallback + 3 exact), assembled by `index.ts` as a choice of one based on mode (never register two identical paths: webserver would throw on duplicates). logout/status behavior is **exactly identical** to M2: logout is POST-only, next is query-only, invokable without body idempotently, `Max-Age=0`; status is GET-only, **only recognizes the cookie** (Bearer session token does not participate: a stateless channel does not create sessions). **D22 amendment (2026-09-23):** password mode now registers a fifth route: prefix `/auth` + **4 exact**, the fourth exact being `/auth/password` (registered only when the optional `passwordChange` deps are injected; `index.ts` always injects them in password mode); the "never register two identical paths" rule and every other P16 statement are unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
2. `username = params.get("username") ?? ""`; `password = params.get("password") ?? ""`;
   ...
   `ip = deps.clientIp?.(req) ?? req.socket.remoteAddress ?? ""` (**D19**: the injected resolver
   normalizes the address and, when configured, reads the trusted proxy header).
3. `const check = deps.limiter.check(ip, username === "" ? undefined : username)`;
   ...
   no-store + `info("rate limit exceeded")`; **return (no verify, no counter increment)**. **D20 amendment (2026-09-22):** the 429 body is now the login card HTML (`sendLockout`; the TOTP second stage uses `sendTotpLockout` with the challenge card) carrying `Too many sign-in attempts from this network. Try again in N seconds. There is no password reset, so ask the instance owner if you are stuck.` instead of `text/plain "too many attempts"`; status and `retry-after` unchanged.
...
6. `if (!ok || user === undefined || user.disabled) { deps.limiter.recordFailure(ip, username === "" ? undefined : username);`
   → 401 `"invalid credentials"` + no-store + `info("login rejected")`; return. `}`. **D20 amendment (2026-09-22):** the 401 body is now the login card HTML — `sendInvalidCredentials` fills the error slot with the constant `Invalid username or password.`, echoes the submitted username back HTML-escaped (password never echoed) and prefixes `<title>` with `Error: ` — while the 401 code, `no-store` and this log line stay unchanged. The `username` read in step 2 is trimmed before the lookup in step 5 and before the DUMMY_HASH verification (the password is never trimmed).
...
Structure follows `auth-endpoints.ts` (merged disposer, track collection): 4 routes in the M3 baseline. (**D22 amendment (2026-09-23):** 5 routes in password mode, i.e. prefix `/auth` + **4 exact**, the fourth being `/auth/password`; registered only when the optional `passwordChange` deps are injected):
...
- `disable`: `(await loadUsersFile(file)).snapshot` → not exist → err(`user not found`) + 1;
  ...
  idempotent).
- **D23 amendment (2026-09-23)**: the CLI mutating commands (`add` / `disable` / `role` / `passwd`) all go
  through `mutateUsersFile` (locked RMW + pre-write CAS + `.bak`, see D23), so every `writeUsersFile` above
  means "the atomic write performed inside `mutateUsersFile`"; `readLine` was replaced by `readLines(n)`
  (one readline session, n lines), which is what makes reading the new password twice in `user passwd` work
  (the old per-line interface returned an empty second line on pipes); `disable`'s unknown-user message is
  unified to `user <name> not found` (matching `passwd` / `role` / `totp`).
...
4. POST login failure: verify false → 401 `"invalid credentials"` + `info("login rejected")` +
   ...
   **DUMMY_HASH** → 401; disabled user → verify receives that user's real hash + 401 (P9: three states unified). (**D20 amendment (2026-09-22):** the 401 body is now the HTML card — assert `content-type: text/html; charset=utf-8` and the constant `Invalid username or password.` instead of `text/plain "invalid credentials"`, plus the echoed username and the `Error: ` title prefix.)
...
6. POST login 503: fake loadUsers rejects → 503 `"user store unavailable"` + error log + no failure counted;
   ...
   the second login does not repeat the warn (module-level flag). (**D20:** same HTML card and constant `Invalid username or password.` as item 4.)
...
3. disabled/unknown users: disableduser → 401; `ghost` → 401 (identical `invalid credentials` response body). (**D20 amendment (2026-09-22):** the body is now the identical HTML card with the constant `Invalid username or password.`)
