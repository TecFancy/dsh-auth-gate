...
| M5  | endpoint set       | `GET /auth/login` (self-contained HTML), `POST /auth/login` (urlencoded: `token` + `next`), `POST /auth/logout`, `GET /auth/status` (JSON `{ authenticated: boolean }`). Routing model per M15 (3 exact + 1 prefix fallback; **D22 amendment (2026-09-23):** the routed set is now **4 exact + 1 prefix fallback** in password mode, the fourth exact route being `/auth/password`, while token mode still registers 3 exact); `/auth/status` **only honors cookies** (Bearer does not participate: the stateless channel creates no session, it is not "session state") **D25 (2026-09-24):** see [impl-m2-admin.md](./impl-m2-admin.md) §4.5b                                           |
...
| M15 | routing model      | webserver has **no method routing** (the `exact` table keys only by pathname, duplicate paths throw, tested) → **4 exact routes** in password mode (**D22 amendment (2026-09-23):** `/auth/login`, `/auth/logout`, `/auth/status`, **`/auth/password`** (added by D22, registered only when the optional `passwordChange` deps are injected); token mode keeps **3 exact**) + **1 prefix fallback** (`/auth` → 404, to prevent unregistered `/auth/*` falling to the SPA fallback, M20); each exact handler dispatches internally by `req.method`, non-whitelisted method → `405` + `allow` header + `text/plain`; **D25 (2026-09-24):** see [impl-m2-admin.md](./impl-m2-admin.md) §4.5b |
...
**D22 amendment (2026-09-23):** in password mode the exact set gains a fourth route, `/auth/password`
(`registerPasswordEndpoints` registers it only when the optional `passwordChange` deps are injected;
`index.ts` always injects them in password mode), so that mode now covers 1 prefix + 4 exact. Token
mode is unchanged (1 prefix + 3 exact) and every statement below about the three M2 endpoints still
holds in both modes.

**P2 admin plane (D25, 2026-09-24):** password mode reaches 1 prefix + **6 exact**; the D25
route/endpoint amendments and the review-pass tightenings moved to
[impl-m2-admin.md](./impl-m2-admin.md) §4.5b.

- **prefix `AUTH_PATH_PREFIX` (= `"/auth"`, fallback, M20)**: always `404` + `content-type: text/plain` +
  no-store: unregistered `/auth/*` must not fall to the SPA fallback (exact takes precedence over prefix, does not shadow the exact endpoints below; in password mode that includes the D22 fourth route, `/auth/password`).
...
store.getByToken(token) !== undefined;`
  ...
  **D25 (2026-09-24):** the authenticated body's additive fields moved to
  [impl-m2-admin.md](./impl-m2-admin.md) §4.5b.1.
...
1. Registration shape: 4 routes in token mode (prefix `/auth` + exact `/auth/login`, `/auth/logout`, `/auth/status`, M15); **D22 amendment (2026-09-23):** password mode's `registerPasswordEndpoints` registers a fifth route, exact `/auth/password` (only when the optional `passwordChange` deps are injected, and its exact-route table assertion must fail if they are omitted). **D25 (2026-09-24):** password mode registers 7 routes (1 prefix + 6 exact); the P2/D25 registration-shape additions moved to [impl-m2-admin.md](./impl-m2-admin.md) §5b.
